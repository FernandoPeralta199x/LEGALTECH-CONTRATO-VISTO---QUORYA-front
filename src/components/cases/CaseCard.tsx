...
/** Status do relatório do caso na listagem (metadata.reportStatus → label PT). Mantido LOCAL
 *  (não centralizado em reportLabels): os defaults diferem do reportStatusLabel canônico
 *  ("Pendente" vs "Não gerado"/"Não iniciado") — unificar seria regressão de rótulo. */
...
type CaseCardProps = {
  ...
  onEdit: (legalCase: Case, event: React.MouseEvent) => void;
  onDelete: (legalCase: Case, event: React.MouseEvent) => void;
};

/** Card de caso na listagem. Presentacional: recebe o caso + callbacks de editar/excluir;
 *  as mutações (updateCase/deleteCase) e a navegação permanecem na página. Extraído de
 *  cases/page (fe-struct-02). */
export function CaseCard({ legalCase: c, index, deletingId, onEdit, onDelete }: CaseCardProps) {
  return (
    <Link
